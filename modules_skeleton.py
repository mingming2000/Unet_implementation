            ...
            for i in range(preds.shape[0]):
                ...
                for j in range(temp.shape[0]):
                    for k in range(temp.shape[1]):
                        ...
                        pred_value = temp[j, k].astype(int)
                        label_value = temp_l[j, k].astype(int)

                        temp_rgb[j, k] = np.array(cls_invert[pred_value])
                        temp_label[j, k] = np.array(cls_invert[label_value])
